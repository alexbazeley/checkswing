...
- Refresh per-owner CSV exports in `data/donations/<slug>/` (plus the `_aggregate/` rollups). These are **tracked in git** and regenerated by the `export` step in `refresh.yml`, so the citable CSVs stay current with `master.db`; they filter `counted = 1` (earmark-deduped), matching the dashboard. Regenerate locally with `python -m scripts.cli export`.
