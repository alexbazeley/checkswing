...
- **State refresh** ([`.github/workflows/refresh-state.yml`](.github/workflows/refresh-state.yml)) runs the Phase-4 state refresh monthly (2nd, staggered after the federal run; and on-demand): for each of the 10 adopted states (CA·CAL-ACCESS, TX·TEC, IL·ISBE, WA·PDC, CO·TRACER, AZ·SOS, MN·CFB, FL·FDOE, NY·NYSBOE, PA·DOS) download the official bulk export or query its API → stream + re-ingest → commit `data/state.db`. LFS-free (it never touches `master.db`), so it consumes no LFS bandwidth.
