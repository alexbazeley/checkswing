...
| Column | Type | Notes |
|---|---|---|
| `transaction_id` | TEXT NOT NULL | FEC `transaction_id_number`. **Filer-assigned and unique only WITHIN one filing** — committees reuse it, so it is NOT the row identity. Kept as the citation field people quote and as the join key for `review_queue` / `review_resolutions` / `manual_attributions`; indexed via `idx_donations_txn`. |
| `record_uid` | TEXT, PK part | **v12: the row identity.** `COALESCE(sub_id, transaction_id)` — FEC's globally-unique `sub_id` when present, else the filer id for older records that carry none. Written by `db.record_uid_for()`, which `fetch_fec._dedupe_key()` mirrors one layer earlier. Nullable in DDL only so raw-SQL test fixtures need not supply it; `cli validate` asserts every real row has one, that pairs are unique, and that no writer bypassed the helper. |
...
| `sub_id` | TEXT | v9: FEC's **globally-unique** record id; since v12 it is the basis of `record_uid`, the actual PK. `transaction_id` is filer-assigned and NOT unique across committees; `sub_id` is the authoritative identity `insert_donation` uses to tell a genuine restatement (same `sub_id`) from a cross-committee `transaction_id` collision (different `sub_id` → distinct contributions, not a restatement — see below). NULL on legacy rows whose raw is gone (backfill via `cli backfill-sub-id`); a NULL falls back to the `transaction_id` identity. |

**transaction_id collisions — resolved in v12 (GOVERNANCE.md §1.5).** `transaction_id` is filer-assigned (e.g. `SA11AI.20387`) and unique only within a single filing, so distinct contributions share one routinely. The archive met this defect at three layers and fixed them in order:

1. **Across owners (v11).** Two owners' contributions sharing an id. The old single-column PK made the second collide with the first; before the §1.3 guard it was recorded as a *fabricated* "FEC restatement" that silently removed one owner's donation from every total (two found live 2026-07-19). Fixed by adding `entity_slug` to the PK.
2. **Within one owner (v12).** The SAME owner can have two real contributions sharing an id — observed as same-day pairs to a campaign and its compliance fund (`dewitt-bill` `SA18.1160868`: $300 to John McCain 2008 *and* $2,300 to the McCain-Palin Compliance Fund; `reinsdorf-jerry` `SA17A.939857` likewise). v11 could not represent this. **v12 keys on `record_uid`**, so both are stored — 45 such pairs exist in the archive.
3. **Before the DB ever saw them.** `load_raw_payloads` de-duplicated on `transaction_id`, silently discarding one of any pair *before* the classifier ran, where no guard could catch it — 132 contributions across 24 owners. Fixed alongside v12 (`fetch_fec._dedupe_key`), which could only land once the schema could store the result.

`insert_donation` still returns `("collision", …)` — writing nothing rather than superseding the wrong record — when an existing row shares an identity but is clearly a different contribution (**recipient committee AND amount both differ**). That condition was tightened in v12: it previously required committee AND date AND amount to *all* differ, so the same-day pairs above slipped through to the supersede path. A **legacy bridge** in `insert_donation` adopts pre-`sub_id` rows (58% of the archive at v12) when the same contribution re-arrives carrying a `sub_id`, upgrading them in place instead of duplicating. The `review_queue` PK is `(transaction_id, entity_slug)` as of v9, so two owners can each flag the same FEC transaction.
